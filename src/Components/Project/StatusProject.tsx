...
export default function StatusProject({ keyword }: PropsType) {
  ...
  const selectTag = (keyword: string) => {
    switch (keyword) {
      case "대기": {
        return <StatusTag text="대기" />
      }
      case "승인": {
        return <StatusTag text="승인" bgColor={color.blue[100]} textColor={color.blue[400]} />
      }
      case "반려": {
        return <StatusTag text="반려" bgColor={color.red[100]} textColor={color.red[300]} />
      }
    }
  }
